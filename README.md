...
Cypher-DDS is developed and tested as a Linux terminal app first. Two other build targets exist, both produced by GitHub Actions rather than locally (PyInstaller has to run on the target OS, and Buildozer needs a full Android SDK/NDK toolchain — neither is available in this project's dev environment). Both workflows have been pushed and watched through to a **confirmed green CI build**, not just written and assumed to work:
